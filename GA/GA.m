...
for i=1:nPop
    ...
    r = randi([1,Network.nSensors],1, 15);
    pop(i).Status = zeros(1,Network.nSensors);
    pop(i).Status(r)=1;
    pop(i).ConsEnergy = ObjectiveFunction(pop(i), Network , Positions, T);
...
for it=1:MaxIt
    ...
    for k=1:nc/2
        ...
        % Evaluate Offsprings
        popc(k,1).ConsEnergy = ObjectiveFunction(popc(k,1), Network , Positions, T);
        popc(k,2).ConsEnergy = ObjectiveFunction(popc(k,2), Network , Positions, T);
    ...
    for k=1:nm
        ...
        % Evaluate Mutant
        popm(k).ConsEnergy=ObjectiveFunction(popm(k), Network , Positions, T);
...
[~, Network] = ObjectiveFunction (BestSol,Network, Positions, T);
